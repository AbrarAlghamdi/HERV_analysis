...
# --------- Inputs ----------
file_01 = "merged_final_count_matrix_common_finalcount_1.tsv"    # 1% FBS (stress)
file_10 = "merged_final_count_matrix_common_finalcount_10.tsv"   # 10% FBS (control)
...
# --------- Load merged count matrices ----------
# These files usually have transcript IDs in column 0 (index)
df01 = pd.read_csv(file_01, sep="\t", index_col=0)
df10 = pd.read_csv(file_10, sep="\t", index_col=0)
...
if len(common) == 0:
    raise RuntimeError("No common transcripts between the two matrices.")
...
# --------- FDR correction ----------
_, padj, _, _ = multipletests(res["p_value"].values, method="fdr_bh")
...
plt.scatter(x[~is_sig], y[~is_sig], alpha=0.6, s=12, edgecolors="none")
plt.scatter(x[is_sig], y[is_sig], alpha=0.8, s=12, edgecolors="none")
